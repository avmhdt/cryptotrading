...
from private.systems.crypto_2024.accounts.account_inputs import accountInputs
...
class accountCosts(accountInputs):
    ...
    def get_SR_cost_for_instrument_forecast(
        ...
        holding_cost = 0    # self.get_SR_holding_cost_only(instrument_code)
    ...
    def get_SR_cost_given_turnover(
        ...
        SR_cost_holding = 0 # self.get_SR_holding_cost_only(instrument_code)
    ...
    def _recent_average_annual_price_vol(self, instrument_code: str) -> float:
        average_vol = self._recent_average_vol(instrument_code)

        avg_annual_vol = average_vol * ROOT_MINUTES_IN_A_YEAR

        return avg_annual_vol

    @diagnostic()
    def _recent_average_vol(self, instrument_code: str) -> float:
        vol = self.get_returns_volatility(instrument_code)
        start_date = self._date_one_month_before_end_of_price_index(instrument_code)
        average_vol = float(vol[start_date:].mean())

        return average_vol

    @diagnostic()
    def _recent_average_price(self, instrument_code: str) -> float:
        minute_price = self.get_final_prices(instrument_code)
        start_date = self._date_one_month_before_end_of_price_index(instrument_code)
        average_price = float(minute_price.loc[start_date:].mean())

        return average_price

    @diagnostic()
    def _date_one_month_before_end_of_price_index(self, instrument_code: str):
        minute_price = self.get_instrument_prices_for_position_or_forecast(
            instrument_code
        )

        last_date = minute_price.index[-1]
        start_date = minute_price.index.asof(last_date - pd.DateOffset(months=1))
