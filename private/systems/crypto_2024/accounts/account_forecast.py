...
from private.systems.crypto_2024.accounts.account_costs import accountCosts
...
def pandl_for_instrument_forecast(
    ...
    root_minutes_in_a_bar: float = 1.0,
    barsize: str = '1 min',
) -> accountCurve:
    if minute_returns_volatility is arg_not_supplied:
        minute_returns_volatility = robust_minute_vol_given_price(price)
    ...
    average_notional_position = _get_average_notional_position(
        minute_returns_volatility,
        risk_target=risk_target,
        value_per_point=value_per_point,
        capital=capital,
        root_minutes_in_a_bar=root_minutes_in_a_bar,
    )
    ...
    account_curve = accountCurve(pandl_calculator, frequency=barsize)
...
def _get_average_notional_position(
    minute_returns_volatility: pd.Series,
    capital: float = ARBITRARY_FORECAST_CAPITAL,
    risk_target: float = ARBITRARY_FORECAST_ANNUAL_RISK_TARGET_PERCENTAGE,
    value_per_point=ARBITRARY_VALUE_OF_PRICE_POINT,
    root_minutes_in_a_bar: float = 1.0,
) -> pd.Series:
    minute_risk_target = risk_target / (ROOT_MINUTES_IN_A_YEAR/root_minutes_in_a_bar)
    minute_cash_vol_target = capital * minute_risk_target

    instrument_currency_vol = minute_returns_volatility * value_per_point
    average_notional_position = minute_cash_vol_target / instrument_currency_vol
