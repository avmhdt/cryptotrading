...
from syscore.dateutils import ROOT_BDAYS_INYEAR, ROOT_MINUTES_IN_A_YEAR
from syscore.constants import arg_not_supplied
from syscore.pandas.strategy_functions import spread_out_annualised_return_over_periods
from sysquant.estimators.vol import robust_minute_vol_given_price
from private.systems.crypto_2024.accounts.pandl_calculators.pandl_generic_costs import (
    pandlCalculationWithGenericCosts,
)
from private.systems.crypto_2024.accounts.pandl_calculators.pandl_calculation import apply_weighting
...
class pandlCalculationWithSRCosts(pandlCalculationWithGenericCosts):
    def __init__(
        ...
        *args,
        ...
        minute_returns_volatility: pd.Series = arg_not_supplied,
        root_minutes_in_a_bar: float,
        **kwargs,
    ):
        ## Is SR_cost a negative number?
        super().__init__(*args, **kwargs)
        self._SR_cost = SR_cost
        self._minute_returns_volatility = minute_returns_volatility
        self._root_minutes_in_a_bar = root_minutes_in_a_bar
    ...
    def weight(self, weight: pd.Series):
        ...
        return pandlCalculationWithSRCosts(
            ...
            minute_returns_volatility=self.minute_returns_volatility,
    ...
    def annualised_price_volatility_points(self) -> pd.Series:
        return self.minute_price_volatility_points * ROOT_MINUTES_IN_A_YEAR / self.root_minutes_in_a_bar

    @property
    def root_minutes_in_a_bar(self):
        return self._root_minutes_in_a_bar

    @property
    def minute_price_volatility_points(self) -> pd.Series:
        minute_price_volatility = self.minute_returns_volatility
        if minute_price_volatility is arg_not_supplied:
            minute_price_volatility = robust_minute_vol_given_price(self.price)

        return minute_price_volatility

    @property
    def minute_returns_volatility(self) -> pd.Series:
        return self._minute_returns_volatility
