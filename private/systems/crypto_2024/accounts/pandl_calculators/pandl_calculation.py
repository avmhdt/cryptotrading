...
from syscore.dateutils import from_config_frequency_pandas_resample, MINUTE_FREQ, from_config_frequency_to_frequency
...
class pandlCalculation(object):
    def __init__(
        ...
        if isinstance(capital, float) or isinstance(capital, int):
            self._capital = pd.Series([capital]*len(price.index), index=pd.DatetimeIndex(price.index))
        else:
    ...
    def capital_as_pd_series_for_frequency(
        self, frequency: Frequency = MINUTE_FREQ
    ) -> pd.Series:
    ...
    def as_pd_series_for_frequency(
        self, frequency: Frequency = MINUTE_FREQ, **kwargs
    ) -> pd.Series:
