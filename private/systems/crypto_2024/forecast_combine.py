...
from syscore.pandas.frequency import reindex_last_weekly_include_first_date
...
class ForecastCombine(SystemStage):
    ...
    def get_combined_forecast(self, instrument_code: str) -> pd.Series:
        ...
        min_abs_forecast = self.get_min_abs_forecast()
        combined_forecast = combined_forecast.where(
            combined_forecast.abs().ge(min_abs_forecast, fill_value=0.0), 0.0
        )
    ...
    def get_weighted_forecasts_without_multiplier(
        ...
        smoothed_minute_forecast_weights = self.get_forecast_weights(instrument_code)
        smoothed_forecast_weights = smoothed_minute_forecast_weights.reindex(
    ...
    def get_forecast_weights(self, instrument_code: str) -> pd.DataFrame:
        # These will be in minute frequency
        minute_forecast_weights_fixed_to_forecasts_unsmoothed = (
            self.get_unsmoothed_forecast_weights(instrument_code)
        )

        # smooth out weights
        forecast_smoothing_ewma_span = self.config.forecast_weight_ewma_span
        smoothed_minute_forecast_weights = (
            minute_forecast_weights_fixed_to_forecasts_unsmoothed.ewm(
                span=forecast_smoothing_ewma_span
            ).mean()
        )

        # change rows so weights add to one (except for special case where all zeros)
        smoothed_normalised_minute_weights = weights_sum_to_one(
            smoothed_minute_forecast_weights
        )

        # still minute

        return smoothed_normalised_minute_weights
    ...
    def get_unsmoothed_forecast_weights(self, instrument_code: str):
        ...
        weekly_forecast_weights = self.get_raw_weekly_forecast_weights(
            instrument_code
        )
        ...
        forecast_weights_fixed_to_forecasts = self._fix_weights_to_forecasts(
            instrument_code=instrument_code,
            weekly_forecast_weights=weekly_forecast_weights,
        )

        # # Remap to minute frequency so the smoothing makes sense also space saver
        # forecast_weights_fixed_to_forecasts = (
        #     forecast_weights_fixed_to_forecasts.resample("1min").mean()
        # )
    ...
    def _fix_weights_to_forecasts(
        self, instrument_code: str, weekly_forecast_weights: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        # the wrong answer
        rule_variation_list = list(weekly_forecast_weights.columns)
        ...
        forecast_weights_fixed_to_forecasts = fix_weights_vs_position_or_forecast(
            weekly_forecast_weights, forecasts
        )
    ...
    @dont_cache
    def get_raw_weekly_forecast_weights(self, instrument_code: str) -> pd.DataFrame:
        """
        ...
        if self._use_estimated_weights():
            forecast_weights = self.get_weekly_raw_forecast_weights_estimated(
                instrument_code
            )
        else:
    ...
    def get_weekly_raw_forecast_weights_estimated(
        self, instrument_code: str
    ) -> pd.DataFrame:
        """
        ...
        """
        optimiser = self.calculation_of_raw_estimated_weekly_forecast_weights(
            instrument_code
        )
    ...
    def calculation_of_raw_estimated_weekly_forecast_weights(self, instrument_code):
        """
    ...
    def get_raw_fixed_forecast_weights(self, instrument_code: str) -> pd.DataFrame:
        ...
        ## Should be weekly for consistency, but span all data
        weekly_forecast_weights = reindex_last_weekly_include_first_date(
            forecast_weights
        )

        return weekly_forecast_weights
    ...
    @input
    def get_min_abs_forecast(self) -> float:
        return self.forecast_scale_cap_stage.get_min_abs_forecast()
