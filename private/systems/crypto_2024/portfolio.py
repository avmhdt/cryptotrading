...
from syscore.dateutils import ROOT_MINUTES_IN_A_YEAR
...
from private.systems.crypto_2024.risk_overlay import get_risk_multiplier
...
class Portfolios(SystemStage):
    ...
    def get_instrument_weights(self) -> pd.DataFrame:
        ...
        minute_unsmoothed_instrument_weights = (
            self.get_unsmoothed_instrument_weights_fitted_to_position_lengths()
        )

        # smooth to avoid jumps when they change
        smoothed_instrument_weights = minute_unsmoothed_instrument_weights.ewm(
    ...
    def get_unsmoothed_instrument_weights_fitted_to_position_lengths(
        ...
        # instrument_weights = instrument_weights.resample("1min").mean()
    ...
    def percentage_return_for_instrument(self, instrument_code) -> pd.Series:
        return self.rawdata.get_percentage_returns(instrument_code)
    ...
    def annualised_percentage_vol(self, instrument_code: str) -> pd.Series:
        vol = self.percentage_vol100scale(instrument_code)
        return (ROOT_MINUTES_IN_A_YEAR / self.root_minutes_in_a_bar)* vol / 100.0
    ...
    ## INPUTS
    def percentage_vol100scale(self, instrument_code: str) -> pd.Series:
        return self.rawdata.get_percentage_volatility(instrument_code)
    ...
    def root_minutes_in_a_bar(self):
        return self.position_size_stage.root_minutes_in_a_bar
